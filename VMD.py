...
# filename = r'start.txt'
# f = pd.read_csv(filename, usecols=['wind_speed'])
filename = r'power.csv'
f = pd.read_csv(filename, usecols=['power'])
plt.plot(f.values)
# alpha = 7000  # moderate bandwidth constraint
# alpha = 1229.97#startk=3
# alpha = 992.17#powerk=3
# alpha = 2828.18#electric k=4
alpha = 2911.63# electric残差4 2911.63
...
for i in range(len(K_)):
    ...
    dataframe.to_csv(r"%d个IMF中心频率-%d.csv" % ((i + 1), (i + 1)), index=False, sep=',')
u, u_hat, omega = VMD(f.values, alpha, tau, K_[3], DC, init, tol)
K = K_[3]
...
plt.xlabel('Number of samples/piece')
plt.plot(f.values)

# fig1.suptitle('原始电功率趋势图')
fig1.suptitle('Run chart of original electric power')
...
for i in range(K):
    ...
    print('执行第%d个子序列' % i)
    dataframe = pd.DataFrame({'v{}'.format(i + 1):a})
    dataframe.to_csv(r"VMDban-%d.csv" % (i + 1), index=False, sep=',')
    # dataframe.to_csv(r"VMDban.csv" , index=False)
